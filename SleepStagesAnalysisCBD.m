...
[b,a] = butter(3,Wn); %Filter coefficients for low pass filter

%We  store the data in STRUC arrays, and not matrices, because every rat
%has a different recording length. To access an element in a STRUCT array,
%use STRUCT{x}
HPC = struct([]);
PFC = struct([]);
BPL = struct([]); %Below the pyramidal layer (HPC)
STATES = struct([]); %PCA scorer output

%Batch 1 - rats 2 3 4 5 9 10 11

tic

disp('Batch 1 rat 2')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat2\2019-04-25_12-03-17\102_CH11.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat2\2019-04-25_12-03-17\102_CH56.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat2\2019-04-25_12-03-17\102_CH16.continuous');
HPC{1} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{1} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{1} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_1\Rat2\states2');
STATES{1} = tmp3.clusters;

disp('Batch 1 rat 3')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat3\2019-05-01_11-53-38\102_CH13.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat3\2019-05-01_11-53-38\102_CH54.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat3\2019-05-01_11-53-38\102_CH20.continuous');
HPC{2} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{2} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{2} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_1\Rat3\states');
STATES{2} = tmp3.clusters;

disp('Batch 1 rat 4')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat4\2019-05-02_11-55-59\102_CH13_2.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat4\2019-05-02_11-55-59\102_CH50_2.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat4\2019-05-02_11-55-59\102_CH17_2.continuous');
HPC{3} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{3} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{3} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_1\Rat4\states');
STATES{3} = tmp3.clusters;

disp('Batch 1 rat 5')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat5\2019-05-03_11-53-36\102_CH11.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat5\2019-05-03_11-53-36\102_CH40.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat5\2019-05-03_11-53-36\102_CH15.continuous');
HPC{4} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{4} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{4} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_1\Rat5\states');
STATES{4} = tmp3.clusters;

disp('Batch 1 rat 9')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat9\2019-05-20_16-19-45\102_CH37.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat9\2019-05-20_16-19-45\102_CH12.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat9\2019-05-20_16-19-45\102_CH50.continuous');
HPC{5} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{5} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{5} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_1\Rat9\states');
STATES{5} = tmp3.clusters;

disp('Batch 1 rat 10')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat10\2019-05-21_15-20-51\102_CH36.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat10\2019-05-21_15-20-51\102_CH15.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat10\2019-05-21_15-20-51\102_CH50.continuous');
HPC{6} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{6} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{6} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_1\Rat10\states');
STATES{6} = tmp3.clusters;

disp('Batch 1 rat 11')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat11\2019-05-23_15-45-04\102_CH35.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat11\2019-05-23_15-45-04\102_CH13.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes\rat11\2019-05-23_15-45-04\102_CH47.continuous');
HPC{7} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{7} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{7} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_1\Rat11\states');
STATES{7} = tmp3.clusters;

toc

%Batch 2 - rats 201 203 204 205 206 207 209 210 211 212 213 214

tic

disp('Batch 2 rat 201')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\201\2020-04-01_12-26-15\102_CH59.continuous.recons.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\201\2020-04-01_12-26-15\102_CH15.continuous.recons.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\201\2020-04-01_12-26-15\102_CH52.continuous.recons.continuous');
HPC{8} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{8} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{8} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat201\states2');
STATES{8} = tmp3.clusters;

disp('Batch 2 rat 203')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\203\2020-04-02_12-21-50\102_CH59.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\203\2020-04-02_12-21-50\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\203\2020-04-02_12-21-50\102_CH36.continuous');
HPC{9} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{9} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{9} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat203\states2');
STATES{9} = tmp3.clusters;

disp('Batch 2 rat 204')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\204\2020-04-03_11-24-46\102_CH43.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\204\2020-04-03_11-24-46\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\204\2020-04-03_11-24-46\102_CH61.continuous');
HPC{10} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{10} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{10} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat204\states2');
STATES{10} = tmp3.clusters;

disp('Batch 2 rat 205')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\205\2020-04-07_11-29-26\102_CH43.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\205\2020-04-07_11-29-26\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\205\2020-04-07_11-29-26\102_CH54.continuous');
HPC{11} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{11} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{11} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat205\states2');
STATES{11} = tmp3.clusters;

disp('Batch 2 rat 206')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\206\2020-04-08_11-22-44\102_CH43.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\206\2020-04-08_11-22-44\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\206\2020-04-08_11-22-44\102_CH61.continuous');
HPC{12} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{12} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{12} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat206\states2');
STATES{12} = tmp3.clusters;

disp('Batch 2 rat 207')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\207\2020-04-09_11-35-16\102_CH38.continuous.recons.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\207\2020-04-09_11-35-16\102_CH7.continuous.recons.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\207\2020-04-09_11-35-16\102_CH49.continuous.recons.continuous');
HPC{13} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{13} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{13} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat207\states2');
STATES{13} = tmp3.clusters;

disp('Batch 2 rat 209')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\209\2020-04-16_11-29-09\102_CH57.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\209\2020-04-16_11-29-09\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\209\2020-04-16_11-29-09\102_CH45.continuous');
HPC{14} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{14} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{14} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat209\states2');
STATES{14} = tmp3.clusters;

disp('Batch 2 rat 210')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\210\2020-04-17_11-25-39\102_CH63.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\210\2020-04-17_11-25-39\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\210\2020-04-17_11-25-39\102_CH33.continuous');
HPC{15} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{15} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{15} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat210\states2');
STATES{15} = tmp3.clusters;

disp('Batch 2 rat 211')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\211\2020-04-21_11-25-38\102_CH57.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\211\2020-04-21_11-25-38\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\211\2020-04-21_11-25-38\102_CH45.continuous');
HPC{16} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{16} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{16} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat211\states2');
STATES{16} = tmp3.clusters;

disp('Batch 2 rat 212')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\212\2020-04-22_11-24-53\102_CH53.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\212\2020-04-22_11-24-53\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\212\2020-04-22_11-24-53\102_CH41.continuous');
HPC{17} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{17} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{17} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat212\states2');
STATES{17} = tmp3.clusters;

disp('Batch 2 rat 213')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\213\2020-04-23_11-20-57\102_CH63.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\213\2020-04-23_11-20-57\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\213\2020-04-23_11-20-57\102_CH33.continuous');
HPC{18} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{18} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{18} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat213\states2');
STATES{18} = tmp3.clusters;

disp('Batch 2 rat 214')
[tmp1, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\214\2020-04-24_11-24-38\102_CH57.continuous');
[tmp2, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\214\2020-04-24_11-24-38\102_CH7.continuous');
[tmp3, ~, ~] = load_open_ephys_data_faster('F:\rat\cannabis\acutes batch 2\214\2020-04-24_11-24-38\102_CH45.continuous');
HPC{19} = downsample(filtfilt(b,a,tmp1),acq_fhz/ds_fhz);
PFC{19} = downsample(filtfilt(b,a,tmp2),acq_fhz/ds_fhz);
BPL{19} = downsample(filtfilt(b,a,tmp3),acq_fhz/ds_fhz);
tmp3 = load('C:\Users\students\Documents\Tugdual\PCA_Scorer_batch_2\Rat214\states2');
STATES{19} = tmp3.clusters;
...
%  save copies of the original data (because it takes almost an hour to
%  load), in case I want to come back to original data
HPC_copy = HPC;
PFC_copy = PFC;
STATES_copy = STATES;
BPL_copy = BPL;
...
STATES{2} = sta3;
STATES{18} = sta213;
%% Manually scored data for batch 1
% Manual scoring which we will use for assessing accuracy of the scorer
% The manually scored data is on a 1sec basis
manual_states = struct([]);
index = 1;
for rats = {'5','9','10','11'}
    path = strcat('C:\Users\students\Documents\Tugdual\CBD_ephys\Rat',rats,'-states');
    path = path{1};
    tmp = load(path);
    manual_states{index} = tmp.states; %structure with several fields, we only take the 'states' one
    index = index+1;
end
...
%Same for the manual scoring, in order to compare, except here 1 data point
%= 1 sec
for ii=1:length(manual_states)
    tmpmanual = manual_states{ii};
    manual_states{ii} = tmpmanual((60*15):end);
end
...
X = categorical({'Rat 1 CBD', 'Rat 3 VEH', 'Rat 4 VEH', 'Rat 5 CBD', 'Rat 9 VEH', 'Rat 10 CBD', 'Rat 11 CBD', 'Rat 201 VEH', 'Rat 203 VEH', 'Rat 204 CBD', 'Rat 205 CBD', 'Rat 206 VEH', 'Rat 207 CBD', 'Rat 209 CBD', 'Rat 210 VEH', 'Rat 211 VEH', 'Rat 212 CBD', 'Rat 213 VEH', 'Rat 214 CBD'});
X = reordercats(X, {'Rat 1 CBD',  'Rat 5 CBD',  'Rat 10 CBD', 'Rat 11 CBD', 'Rat 204 CBD','Rat 205 CBD','Rat 207 CBD', 'Rat 209 CBD', 'Rat 212 CBD','Rat 214 CBD', 'Rat 3 VEH', 'Rat 4 VEH','Rat 9 VEH', 'Rat 201 VEH', 'Rat 203 VEH',  'Rat 206 VEH', 'Rat 210 VEH', 'Rat 211 VEH', 'Rat 213 VEH'});
...
%% Accuracy scores
% This section assesses the accuracy of the scorer by comparing manual
% scoring with PCA-scoring
states_amplified = struct([]);
index = 1;
for ii = [4 5 6]  % corresponds to rats 5, 9 ,10 from batch 1, which were manually scored
    tmp = repelem(STATES{ii}, 10); % we amplify to match the manual scoring in length, so basically x10 each element
    states_amplified{index} = tmp(1:length(manual_states{index})); %Fit the same length exactly (as the manual scoring might have a few seconds more of scoring)
    index = index+1;
end

states_amplified{index} = repelem(STATES{7},10);  %Rat 11 is slightly different, as we removed data at the end also because of an artifact, so in this case we need to shorten the manual scoring
tmp = manual_states{index};
manual_states{index} = tmp(1:length(states_amplified{index}));

% Now set the same values for the different states
Global_accuracy = [];
CM = struct([]);
for ii=1:4
    tmp1 = states_amplified{ii};
    tmp2 = manual_states{ii};
    tmp2(tmp2 == 5) = 1; %REM
    tmp2(tmp2 == 3) = 2; %NREM
    tmp1(tmp2 == 0) = [];
    tmp2(tmp2 == 0) = [];
    Global_accuracy(ii) = sum(tmp1 == tmp2) / length(tmp1);
    CM{ii} = confusionmat(tmp1, tmp2);
end
...
hold on
% Individual dots
scatter(ones(1,10)*0.5,anova_mat_rem2(1:10),[],'black', 'markerfacecolor', [0.4667    0.7804    0.3529], 'MarkerFaceAlpha', 0.5, 'MarkerEdgeAlpha', 0.3)
hold on
scatter(ones(1,9)*1.5,anova_mat_rem2(11:19),[],'black', 'markerfacecolor', [0.5020    0.5020    0.5020], 'MarkerFaceAlpha', 0.5, 'MarkerEdgeAlpha', 0.3)
% Error bars
...
for ii = 1:size(mat_rem_boutsnb,1)
    tmp1 = mat_rem_boutsnb(ii,:);
    tmp2 = mat_rem_boutsdur(ii,:);
    n = N(ii,:)
    mat_rem_boutsnb2(ii) = mean(tmp1(n));
    mat_rem_boutsdur2(ii) = mean(tmp2(n));
end
[H_rem_boutsnb,P_rem_boutsnb,CI_rem_boutsnb,STATS_rem_boutsnb] = ttest2(mat_rem_boutsnb2(1:10), mat_rem_boutsnb2(11:19))
[H_rem_boutsdur,P_rem_boutsdur,CI_rem_boutsdur,STATS_rem_boutsdur] = ttest2(mat_rem_boutsdur2(1:10), mat_rem_boutsdur2(11:19))
...
for ii = 1:size(ttest_mat_nrem,1)
    tmp = ttest_mat_nrem(ii,:);
    n = N(ii,:)
    ttest_mat_nrem2(ii) = mean(tmp(n));
end
[H_nrem,P_nrem,CI_nrem,STATS_nrem] = ttest2(ttest_mat_nrem2(1:10), ttest_mat_nrem2(11:19))
...
for ii = 1:size(mat_nrem_boutsnb,1)
    tmp1 = mat_nrem_boutsnb(ii,:);
    tmp2 = mat_nrem_boutsdur(ii,:);
    n = N(ii,:)
    mat_nrem_boutsnb2(ii) = mean(tmp1(n));
    mat_nrem_boutsdur2(ii) = mean(tmp2(n));
end
[H_nrem_boutsnb,P_nrem_boutsnb,CI_nrem_boutsnb,STATS_nrem_boutsnb] = ttest2(mat_nrem_boutsnb2(1:10), mat_nrem_boutsnb2(11:19))
[H_nrem_boutsdur,P_nrem_boutsdur,CI_nrem_boutsdur,STATS_nrem_boutsdur] = ttest2(mat_nrem_boutsdur2(1:10), mat_nrem_boutsdur2(11:19))
...
for ii = 1:size(mat_all_boutsnb,1)
    tmp1 = mat_all_boutsnb(ii,:);
    tmp2 = mat_all_boutsdur(ii,:);
    n = N(ii,:)
    mat_all_boutsnb2(ii) = mean(tmp1(n));
    mat_all_boutsdur2(ii) = mean(tmp2(n));
end
[H_all_boutsnb,P_all_boutsnb,CI_all_boutsnb,STATS_all_boutsnb] = ttest2(mat_all_boutsnb2(1:10), mat_all_boutsnb2(11:19))
[H_all_boutsdur,P_all_boutsdur,CI_all_boutsdur,STATS_all_boutsdur] = ttest2(mat_all_boutsdur2(1:10), mat_all_boutsdur2(11:19))
...
for ii = 1:size(ttest_mat_trans,1)
    tmp = ttest_mat_trans(ii,:);
    n = N(ii,:)
    ttest_mat_trans2(ii) = mean(tmp(n));

end
[H_trans,P_trans,CI_trans,STATS_trans] = ttest2(ttest_mat_trans2(1:10), ttest_mat_trans2([11 13:19]))
...
%t-test with averaged over whole time
for ii = 1:size(mat_ripples,1)
    tmp = mat_ripples(ii,:);
    mat_ripples2(ii) = mean(tmp(~isnan(tmp)));
end
[H_ripples,P_ripples,CI_ripples,STATS_ripples] = ttest2(mat_ripples2(1:10), mat_ripples2(11:19))
...
for ii = 1:size(mat_sw,1)
    tmp = mat_sw(ii,:);
    mat_sw2(ii) = mean(tmp(~isnan(tmp)));
end
[H_sw,P_sw,CI_sw,STATS_sw] = ttest2(mat_sw2(1:10), mat_sw2(11:19))
...
for ii = 1:size(mat_theta,1)
    tmp = mat_theta(ii,:);
    mat_theta2(ii) = mean(tmp(~isnan(tmp)));
end
[H_theta,P_theta,CI_theta,STATS_theta] = ttest2(mat_theta2(1:10), mat_theta2(11:19))
...
for ii = 1:size(mat_delta,1)
    tmp = mat_delta(ii,:);
    mat_delta2(ii) = mean(tmp(~isnan(tmp)));
end
[H_delta,P_delta,CI_delta,STATS_delta] = ttest2(mat_delta2(1:10), mat_delta2(11:19))
