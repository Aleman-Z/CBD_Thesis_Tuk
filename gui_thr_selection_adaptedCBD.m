...
DATA = load('C:\Users\students\Documents\Tugdual\PLOTS\Rat5\filtered_data');
...
STATES = load('C:\Users\students\Documents\Tugdual\PLOTS\Rat5\statesPCA');
STATES = STATES.STATES;
states = STATES{1};
states(states==3) = 2;  %make intermediate state to NREM for the ripple detection

HPC = HPC(1:length(states));
PFC = PFC(1:length(states));
states = states(1:600:end);

    %Convert signal to 1 sec epochs.
    e_t=1;
    e_samples=e_t*(600); %fs=600 Hz
    ch=length(HPC);
    ...
    vec_bin=states;
    vec_bin(vec_bin~=2)=0;  %REM
    vec_bin(vec_bin==2)=1;  %NREM
    %Cluster one values:
...
%%
    ...
    dlgtitle = ['Threshold PFC']; %'Threshold PFC';
    definput = {'40'};
    % opts.Interpreter = 'tex';
    answer = inputdlg(prompt,dlgtitle,[1 40],definput);
    D2=str2num(answer{1}) 
...
yticklabels({'HPC','PFC','HPC (100-300Hz)', 'PFC (100-300Hz)'})
